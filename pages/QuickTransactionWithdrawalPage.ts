...
// Page Object for Quick Transactions — Withdrawal transaction type scope.
// "Transfer to Account" field is not present for Withdrawal (hidden in DOM via #transferAccountGroup).
// Uses human-like pauses between field interactions for easy visual monitoring.

export class QuickTransactionWithdrawalPage extends BasePage {
  ...
  async selectWithdrawalTransactionType(): Promise<void> {
    await this.pause();
    await this.selectOption(this.transactionTypeDropdown, 'Withdrawal');
    await this.pause();
  }
